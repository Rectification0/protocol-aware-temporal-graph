...
A React SOC dashboard, planned across Milestones F0–F17 in `tasks.md`'s "Frontend Implementation" section (only exists on this branch). **Milestones F0–F15 are complete**; F16–F17 remain — check `tasks.md` for current status before starting new frontend work.
...
**F15 — Settings.** No backend changes — every setting is a client-side preference or a read-only view of an already-existing endpoint. New `features/settings/` folder (`ThemeSection`, `NotificationSection`, `ApiConfigSection`, `AlertThresholdsSection` + `logic.ts`) assembled onto `SettingsPage`. Three new/retrofitted Zustand stores, all persisted to `localStorage` via `zustand/middleware`'s `persist` (F15.6's actual scope — not a seventh setting, the persistence layer the other three use): `themeStore` (F15.1 — `useAppliedTheme()` really does toggle `<html>`'s `.dark` class, but it's a documented no-op today since `index.css`'s `:root`/`.dark` share the same verified-dark values pending F16.2's real light palette) and `notificationSettingsStore` (F15.2 — narrows `features/monitoring/logic.ts`'s new `filterEnabledAlertEvents`/`countUnreadEnabledAlerts`, consumed by `NotificationsPanel`/`useLiveNotifications`; F13.5's Critical Alerts panel is deliberately unaffected, per tasks.md F15.2's own wording) are new; `autoRefreshStore` (F13.3) gained `persist` retrofitted onto it rather than leaving it the odd one out. F15.3 (API config) and F15.5 (alert thresholds) are both read-only by design — the former because changing the base URL means reconnecting to a different backend process entirely, not a client preference; the latter because `config/protocols.yaml`/`config/motifs.yaml` are still hand-edited + `reload()`'d (`docs/operational-runbook.md`), with an editable UI explicitly out of this milestone's scope.
...
**F6–F15 feature folders**, each with a pure `logic.ts` + thin components, one per page/section: `features/dashboard/`, `features/analytics/` (also owns `timeRangeStore.ts`, shared from F8 onward by F7/F9/F11/F12), `features/detections/` (severity derivation reused by F11/F13), `features/investigation/`, `features/logs/`, `features/monitoring/` (owns `notificationsStore`, `alertAckStore`, `autoRefreshStore`, `notificationSettingsStore`; `AppShell` is the single `useLiveStream()` owner for the whole app), `features/security-overview/`, `features/settings/` (F15 — `themeStore` is its other store, applied app-wide via `useAppliedTheme()` in `AppShell` rather than scoped to the Settings page itself). Recurring conventions worth knowing before adding a new tile/page: reuse an existing threshold/derivation rather than inventing a second one when two pages need the same "is this bad" judgment; name the exact `[BACKEND TODO]` gap (F0.12/F0.13/F0.14) via `BackendPendingState` rather than fabricating data; caption every provisional/sampled/non-live number in real UI copy, not just a code comment; a client-side preference that should survive a reload gets `zustand/middleware`'s `persist` (localStorage, keys prefixed `t-gnn-`) — first established in F15.6.
