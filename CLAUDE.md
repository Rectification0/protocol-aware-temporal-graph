...
Milestone F13 (Live Monitoring) is fully implemented: F13.1-F13.6 are all
real, verified code (F13.6 was previously `[~]` partial -- its API side
landed back in F0 -- and is now `[x]`, the frontend consumption tasks.md's
own note said was still missing). No backend changes were needed; every
F13 task is a frontend consumer of API surface F0/F4 already exposed. The
one genuine architectural fix: F13.1's own line asked for real-time
wiring "across F6/F7/F9's live tiles," but before this milestone the only
place `useLiveStream()` was ever mounted was F7.4's `LiveAttackCounter` --
since that hook opens its own `EventSource` connection per mount, F6's
Overview tiles and F9's Detection Matrix were never actually live-pushed
to at all, even though their query keys are exactly what `liveStream.ts`
already invalidates on `inference_result`/`motif_completion`/
`motif_reset`. `frontend/src/components/AppShell.tsx` is now the single
owner of that connection for the whole authenticated app (gated on a
valid session), and `LiveAttackCounter` was updated to read the same
shared store (`useLiveStream({ enabled: false })`) rather than opening a
second one. A new `frontend/src/features/monitoring/` folder holds
F13.2/F13.4/F13.5/F13.6's components plus their shared `logic.ts` (event
classification/severity/summary/detection-ref derivation, all reusing
F9's existing severity functions rather than a fourth scheme); two new
Zustand stores (`notificationsStore`, `alertAckStore`) back F13.4's
unread-count and F13.6's session-scoped ack state respectively;
`autoRefreshStore` backs F13.3, read by `useMetricsSnapshot`/`useHealth`
(the two hooks with no matching `stream.py` SSE event type) instead of
their previous hardcoded `refetchInterval`. See the Architecture section
addendum below for file-by-file detail.

Milestone F14 (Company Security Overview) is fully implemented: F14.1-F14.4
are all done, though only two of the four needed new frontend components
(and F14.3 needed a real backend addition). F14.1/F14.2 needed no new
component at all -- both lines name F6 tiles (`CybersecurityScoreTile`/
`SecurityLevelTile`, `SystemHealthTile`/`MonitoringStatusTile`) as what to
reuse, and this milestone's own intro line explicitly calls it "largely a
second view over F6/F0.12's data at a different altitude... reuse F6's
hooks rather than duplicating fetch logic" -- so the new
`frontend/src/pages/SecurityOverviewPage.tsx` renders those same tiles
directly, the same reuse-over-duplicate judgment call F12.1/F12.2/F12.4
already made for charts, just applied to whole tiles this time. F14.3's
"processed logs" needed the real backend change its own tasks.md line
anticipated: `MetricsCollector` (`src/t_gnn/metrics.py`) gained a sixth,
lifetime-counter quantity (`total_edges_processed`, distinct from its
five existing rate-based ones), persisted via this repo's first
`ALTER TABLE` migration statement (`metrics_snapshots` already existed in
developers' local databases, so a plain `CREATE TABLE IF NOT EXISTS`
wouldn't have retroactively added the column). F14.3's "monitored users"
and "analyzed sessions" needed no backend work -- the former reuses
F10.1's `/api/entities` count, the latter is a `BackendPendingState`
naming F0.13, the same decision F10.9 already made for the identical
"no session concept exists" gap. F14.4 required deciding a real
definition before building anything (its own line's explicit
instruction) -- resolved as analyst ack latency (F13.6), not
time-to-detection (already covered by the existing inference-latency
metric) -- backed by a new `GET /api/alerts/response-time` that finally
gives `list_alert_acknowledgements()` (real and tested since F13.6, but
with zero callers until now) an actual consumer. See the Architecture
section addendum below for file-by-file detail.
...
**Frontend Implementation: Milestone F13 — Live Monitoring.** No backend changes at all -- every task is a frontend consumer of API surface F0/F4 already exposed (`POST /api/alerts/ack` landed with F0, `useAlertAck()` with F4.2). The one real architectural fix is F13.1's connection-ownership change; everything else is new UI over data F4.6's stream/store already carried.

- F13.1's fix: before this milestone, `useLiveStream()` (F4.6) was only ever mounted by F7.4's `LiveAttackCounter` -- and since that hook opens its own `EventSource` connection per call, F6's Overview tiles and F9's Detection Matrix (whose `useEntityScores`/`useMotifCompletions` query keys are exactly what `liveStream.ts` already invalidates on `inference_result`/`motif_completion`/`motif_reset`) never received a single live push, only their own 5-10s polling. `frontend/src/components/AppShell.tsx` now calls `useLiveStream({ enabled: hasSession })` once, for the whole authenticated app (`isSessionValid(useAuthStore(...))` -- `AppShell` renders even mid-redirect for an unauthenticated visitor, per `ProtectedRoute`'s placement one level down, so this guards against opening a connection before login). `LiveAttackCounter.tsx` was changed to `useLiveStream({ enabled: false })` -- the hook's returned `{status, events, lastHeartbeatAt, lastError}` are always read straight from `useLiveStreamStore` regardless of `enabled`, so this is now a pure read of the shared connection's state, not a second connection. No test changes were needed for `LiveAttackCounter.test.tsx` -- it already mocked the whole `useLiveStream` hook, which doesn't care what argument it's called with.
- `frontend/src/features/monitoring/logic.ts` is the pure-derivation layer, same split every earlier milestone's `logic.ts` established. `eventSeverity()` reuses F9's own `severityFromMotifConfidence()`/`severityFromAnomalyScore()` (`features/detections/logic.ts`) rather than a fourth severity scheme -- a motif completion is always a real detection (never `null`), an `inference_result` only counts once its severity clears `'info'` (the same "not benign" bar F7.1/F9.3 use), and `motif_reset`/`prune` are always `null` (operational/audit events, not detections). `isAlertEvent()`/`filterAlertEvents()` (`eventSeverity() !== null`) back F13.4's notification list and `useLiveNotifications`'s toast trigger; `filterCriticalEvents()` (`eventSeverity() === 'critical'`) backs F13.5. `summarizeLiveEvent()` is the one human-readable description shared by F13.2's raw feed, F13.4's notification list, and `useLiveNotifications`'s toast body. `detectionRefFor()` builds the `{detection_type, detection_ref}` pair matching `alerts.py`'s own documented shape (`{motif_name}:{chain_key}:{completed_at}` / `{entity_id}:{t}`) for the two detection event types, `null` for the other two. `connectionStatusTone()` maps `LiveStreamStatus` onto F6.2-F6.5's `StatusPill` tone vocabulary (`features/dashboard/status-pill.tsx`) -- cross-feature reuse, not a duplicate, since "is the stream connected" is infra status, not a threat severity.
- Two new Zustand stores, single-responsibility-split from `liveStreamStore` (whose own job is raw transport state) the same way `authStore`/`timeRangeStore` are already split from each other: `src/store/notificationsStore.ts` (`lastReadAt`, `markAllRead()` -- F13.4's unread-count baseline) and `src/store/alertAckStore.ts` (`ackedKeys`, `markAcked()`/`isAcked()` -- F13.6's ack state). `alertAckStore` is deliberately session-scoped, not synced from the server: `alerts.py` is POST-only with no GET/list endpoint (`useAlertAck()`'s own pre-existing comment already documents this gap), so there is no way to hydrate acked state on load -- an ack made here is genuinely persisted server-side (`alert_acknowledgements`), but a reload or a different tab won't reflect it until a GET endpoint exists. `src/store/autoRefreshStore.ts` (`enabled`, `intervalMs`) backs F13.3; `useMetricsSnapshot`/`useHealth` (F4.2) -- the two hooks with no matching `stream.py` SSE event type, so they stay polling-only by design -- now read `refetchInterval` from this store (`useHealth` additionally floors at its own pre-existing 15s ceiling, so the shared control's shortest setting can't make health checks poll faster than before) instead of a hardcoded number, making the Monitoring page's control affect both hooks wherever they're mounted, not just while that page is open.
- `frontend/src/features/monitoring/AckButton.tsx` (F13.6) is the frontend half tasks.md's own F13.6 note said was still missing. Renders "Ack" (calling `useAlertAck()`, attributed to the mock-auth analyst) or "Acknowledged" (from `alertAckStore`) -- updates the store from the mutation's own `onSuccess`, the exact pattern that hook's comment already prescribed. Composed into both `LiveEventFeed.tsx` (F13.2) and `CriticalAlertsPanel.tsx` (F13.5) wherever a row/banner has a real `detectionRefFor()` target.
- `frontend/src/features/monitoring/LiveEventFeed.tsx` (F13.2) is a scrollable table (F5's `ScrollArea`/`Table` primitives) of every event in `useLiveStreamStore` (bounded to its own 200-event cap), newest first, with connection status/last-heartbeat/stream-error display and a "Clear feed" button (`useLiveStreamStore.clear()`). Includes `motif_reset` rows alongside the three types tasks.md's own line names, since the store/stream already carry it as a real fourth event type and dropping it from this one panel would be an odd, undocumented gap.
- `frontend/src/features/monitoring/CriticalAlertsPanel.tsx` (F13.5) renders `filterCriticalEvents()`'s subset via F5.10's `AlertBanner`. `AlertBanner` has no built-in slot for extra actions (just `severity`/`title`/`description`/`onDismiss`), so `AckButton` is composed as a footer row beneath each banner rather than changing that shared component's props for one caller.
- `frontend/src/features/monitoring/NotificationsPanel.tsx` (F13.4) is a bell icon + unread badge in the Navbar (shadcn `DropdownMenu`, this repo's first real consumer of that primitive), listing `filterAlertEvents()`'s most recent 10, each with `AckButton`. Its rows are plain `div`s, not `DropdownMenuItem`s -- Radix closes a dropdown menu on any item selection by default, which would dismiss the whole panel the instant an analyst clicked an `AckButton` inside a `DropdownMenuItem`. Opening the dropdown (`onOpenChange`) calls `markAllRead()`. `frontend/src/features/monitoring/useLiveNotifications.ts` is the F5.11 half: an effect hook (mounted once in `AppShell`, not tied to any one page) that toasts each newly-arrived alert event via `@/components/toast`. Its baseline (`lastNotifiedAtRef`) is set to `Date.now()` *inside* the effect on first run, not read during render -- an impure `Date.now()` call in a component body is exactly what F6.7/F6.3's own comments already flag as unsafe under the React Compiler's purity lint (`react-hooks/purity`, active via `eslint-plugin-react-hooks@7`) -- and starting at "now" rather than 0 avoids toasting a burst of already-buffered events (the store persists across page navigations within a session) the moment this hook first mounts.
- `frontend/src/pages/MonitoringPage.tsx` assembles `AutoRefreshControl`, `CriticalAlertsPanel`, and `LiveEventFeed` under real headings (replacing the F13 placeholder). F13.1's connection and F13.4's `NotificationsPanel` are deliberately NOT on this page -- `AppShell`/`Navbar` respectively, since neither is meaningfully page-specific (a bell icon needs to be reachable everywhere, and "is the connection open" isn't a Monitoring-page-only concern once F6/F7/F9's tiles depend on it too).
- Tests: `features/monitoring/logic.test.ts` (severity/alert/critical classification across all four event types, summary text, detection-ref construction matching `alerts.py`'s documented shape, connection-status tone mapping), `store/{notificationsStore,alertAckStore,autoRefreshStore}.test.ts`, one test file per new component (`AckButton`, `LiveEventFeed`, `CriticalAlertsPanel`, `NotificationsPanel` -- including the mark-read-on-open and unread-badge-count behavior -- and `AutoRefreshControl`), `useLiveNotifications.test.ts` (using `renderHook`/`act`, asserting no toast fires for pre-existing buffered events but does fire for a genuinely new one), a `MonitoringPage.test.tsx` smoke test, and `router.test.tsx`/`Navbar.test.tsx` coverage carried over unchanged (both already passed against the new wiring with no test edits beyond `router.test.tsx`'s new `/monitoring` heading case). Verified concretely: `npx tsc -b --noEmit`, `npm run lint`, `npm run format:check`, `npm run build`, and `npm run test` (292 tests across 63 files, up from 249/52 pre-F13) all pass clean. No backend changes this milestone, so the backend `pytest` suite wasn't re-run beyond what F11's pass already confirmed. The dev server was not separately started this pass -- same "no browser-automation tool available" caveat every prior milestone's addendum already notes, so a full visual check of the live-backend rendering (in particular, an actual SSE connection against a running `python -m t_gnn.api`) was not done.

**Frontend Implementation: Milestone F14 — Company Security Overview.** Backend-first for F14.3/F14.4's real gaps, the same shape F8/F10/F11 already took; F14.1/F14.2 needed no code at all beyond a new page reusing existing F6 tiles.

- `src/t_gnn/metrics.py`'s `MetricsCollector` (F14.3) gained `total_edges_processed` (a plain `int` counter) and `observe_edge_processed()` (increments it) -- `scripts/run_pipeline.py`'s `_process_edge()` calls it once per edge, alongside its existing `processor.process()`/`motif_engine.on_edge()` calls. `MetricsSnapshot` gained the matching field (default `0`, so every existing positional-args test construction stays valid). `src/t_gnn/api_state.py`'s `metrics_snapshots` table gained a `total_edges_processed BIGINT NOT NULL DEFAULT 0` column two ways at once: added to the `CREATE TABLE IF NOT EXISTS` (for a fresh database) *and* a new `ALTER TABLE metrics_snapshots ADD COLUMN IF NOT EXISTS ...` statement in `_DDL_STATEMENTS` (for a database where the table already exists, since `CREATE TABLE IF NOT EXISTS` is a no-op there) -- this repo's first schema-migration statement of any kind, since every prior milestone only ever added whole new tables. `record_metrics_snapshot()`/`latest_metrics_snapshot()` and `MetricsSnapshotOut` (schemas.py) were updated to carry the new column through to `GET /api/metrics/snapshot` -- no new endpoint needed.
- `src/t_gnn/api_state.py`'s `ApiStateReader.average_response_time()` (F14.4, new `AlertResponseTimeStats` dataclass) is the first real consumer of `list_alert_acknowledgements()` -- a method that's existed, real and tested, since F13.6, but had no HTTP route or frontend caller until now. It parses each acknowledgement's own `detection_ref` for its embedded detection timestamp via `ref.rsplit(":", 1)[-1]` (both known `detection_ref` shapes -- `{motif_name}:{chain_key}:{completed_at}` and `{entity_id}:{t}` -- always put the timestamp last, regardless of how many `:`-separated segments come before it, e.g. a `Machine:C1042` chain key). Averaging in Python rather than SQL was a deliberate choice: extracting a trailing colon-delimited field via `split_part`'s negative-index form needs Postgres 14+, and this is a small, infrequent aggregate, not a hot path worth a version-specific dependency. New `GET /api/alerts/response-time` (`routers/alerts.py`) returns `{average_seconds, sample_size}` -- `null`/`0`, not a fabricated `0s`, when no acknowledgement has a parseable timestamp yet (i.e., none have been made at all).
- `frontend/src/pages/SecurityOverviewPage.tsx` (new, routed at `/security-overview` -- a genuinely new top-level route not among F2.1's original seven, since this milestone didn't exist when that route table was reserved; `config/routes.ts`'s `ROUTES.securityOverview` and a new `NAV_ROUTES` "Company Overview" entry) assembles: F14.1 (`CybersecurityScoreTile`/`SecurityLevelTile`, F6.1/F6.2, rendered as-is), F14.2 (`SystemHealthTile`/`MonitoringStatusTile`, F6.4/F6.5, rendered as-is, with real page copy -- not just a code comment -- stating the non-SOAR caveat specs.md §4 requires), and F14.3/F14.4's three new tiles in `frontend/src/features/security-overview/`.
- `MonitoredUsersTile.tsx` (F14.3) reuses F10.1's `useEntities({pageIndex: 0, pageSize: 1}, 'User')` purely for its exact `total` -- the same "page size 1 just for the count" trick `AttacksInRangeTile` (F8.3) already uses -- inheriting F10.1's own honest caveat (a cold-storage view; a User with only currently-active edges hasn't been pruned yet and won't count). `ProcessedLogsTile.tsx` reads the new `total_edges_processed` off `useMetricsSnapshot()`, formatted via `.toLocaleString('en-US')` -- an explicit locale, not the runtime's own (a real bug this component's own test caught: an unqualified `toLocaleString()` renders `"1,23,456"` instead of `"123,456"` under a non-en-US ICU locale, and a shared SOC dashboard shouldn't format the same number differently depending on which analyst's machine is looking at it). "Analyzed sessions" is `BackendPendingState` naming F0.13, inline on the page -- same as F10.7-F10.9, no wrapper component needed.
- `AverageResponseTimeTile.tsx` (F14.4) reads the new `useAlertResponseTime()` hook (`hooks/api/`, a 30s `staleTime`, no polling -- acks aren't a fast-moving number) and formats the result via a new `features/security-overview/logic.ts`'s `formatDurationSeconds()` (this milestone's one piece of real pure logic -- everything else is thin `StatCard` wrappers, matching the file's own comment on why it's otherwise sparse). `frontend/src/features/monitoring/AckButton.tsx` (F13.6) was updated to also invalidate `queryKeys.alertResponseTime()` in its mutation's `onSuccess`, alongside the existing `markAcked()` call -- so acknowledging something anywhere in Live Monitoring refreshes this tile's average immediately rather than waiting on its own `staleTime`.
- Tests: `features/security-overview/logic.test.ts` (every duration-formatting boundary, including the negative-clamp and rounding cases), one test file per new tile (mocking `@/api/endpoints` the same way every earlier tile test does, including the locale-formatting regression case for `ProcessedLogsTile`), a `SecurityOverviewPage.test.tsx` smoke test asserting every section/tile renders together, and a `router.test.tsx` addition covering the new `/security-overview` route and `NAV_ROUTES` entry. Backend: `tests/test_metrics.py` gained cases for `total_edges_processed`'s zero-default and increment-not-eviction behavior; `tests/test_api.py`/`test_api_state.py` gained cases for the new `total_edges_processed` field (including updating two pre-existing exact-JSON-equality assertions that would otherwise fail once the field appeared in the response) and `GET /api/alerts/response-time` (empty case, and a real computed-average case; the live-Postgres version uses `limit=1` to deterministically isolate its own just-inserted row from whatever other tests/runs have already left in the shared, uncleaned `alert_acknowledgements` table, the same class of isolation problem F10's forensics tests solved with unique-prefix filtering instead). Verified concretely: the full backend `pytest` suite (308 passed, 2 skipped -- Neo4j/Redis-dependent tests, unrelated; the local `docker compose` stack was paused when this pass started, briefly unpaused to exercise real Postgres/Neo4j/Redis-dependent tests including the new migration against real Postgres, then re-paused afterward), and on the frontend, `npx tsc -b --noEmit`, `npm run lint`, `npm run format:check`, `npm run build`, and `npm run test` (305 tests across 68 files, up from 292/63 pre-F14) all pass clean. The dev server was not separately started this pass -- same "no browser-automation tool available" caveat every prior milestone's addendum already notes.
